    ...
    public static List<Dish> GetDishes(String json, int dishesAmount)
    {
        ...
        if (results.size() > 0)
        {
            for (int i = 0; i < dishesAmount; i++)
            {
                ...
                if (dishInformation.get("usedIngredientCount").getAsInt() != 0)
                { // заходим сюда, если блюдо ищем по ингредиентоам
                    if (dishInformation.get("missedIngredients").getAsJsonArray().size() == 0)
                    {
                        recipe = getRecipe(dishInformation);
                    }
                    else
                    { // выходим из метода, т.к. json отсортирован, т.е. дальше не будет подходящих блюд
                        break;
                    }
                }
                else
                { // заходим сюда, если ищем блюдо по названию
                    recipe = getRecipe(dishInformation);
                }
